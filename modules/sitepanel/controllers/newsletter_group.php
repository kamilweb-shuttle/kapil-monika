...
class Newsletter_group extends Admin_Controller {
	...
	public function add()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
		    ...
		    $this->newsletter_group_model->safe_insert('tbl_newsletter_groups',$posted_data,FALSE);
	...
	public function edit()
	{
		...
		if( is_object($rowdata) && !empty($rowdata) )
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				$this->newsletter_model->safe_update('tbl_newsletter_groups',$posted_data,$where,FALSE);					
				$this->session->set_userdata(array('msg_type'=>'success'));
