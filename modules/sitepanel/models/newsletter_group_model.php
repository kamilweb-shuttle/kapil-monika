...
class Newsletter_group_model extends MY_Model
{
	...
	public function getallrecord($offset=FALSE,$per_page=FALSE)
	{
		...
		$result = $this->findAll('tbl_newsletter_groups',$fetch_config);
		return $result;
	}
	...
	public function get_records_by_id( $newsletter_groups_id )
	{
		...
		if($newsletter_groups_id!='' && is_numeric($newsletter_groups_id)){
			...
			$result = $this->find('tbl_newsletter_groups',$fetch_config);
			return $result;
		}
	}
	...
	public function change_status()
	{
		...
		if( is_array($arr_ids) ){
			...
			if($this->input->post('Activate')=='Activate'){				
				...
				$this->safe_update('tbl_newsletter_groups',$data,$where,FALSE);
			...
			{	
					...
					$this->db->where("newsletter_groups_id IN ($str_ids)");
					$this->db->delete('tbl_newsletter_groups');
	...
	{
		$qry = $this->db->query("SELECT newsletter_groups_id from tbl_newsletter_groups where 1 $cond");
		$num=$qry->num_rows();
		$qry->free_result();
		return $num;
	}
	...
	{								
		...
		$query=$this->db->get('tbl_newsletter_groups');		
	...
	{
	    $this->db->select('tbl_newsletters.subscriber_email');
        $this->db->from('tbl_newsletters');
		$this->db->where('tbl_newsletter_group_subscriber.newsletter_groups_id',$newsletter_groups_id);	
		$this->db->join('tbl_newsletter_group_subscriber',
		 'tbl_newsletters.subscriber_id=tbl_newsletter_group_subscriber.subscriber_id','INNER');	
