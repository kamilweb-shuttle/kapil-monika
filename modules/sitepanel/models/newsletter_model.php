...
class Newsletter_model extends MY_Model{
	...
	{	
		...
		$this->db->from("tbl_newsletters");
		 ...
		 {
		    $this->db->join("tbl_newsletter_group_subscriber","tbl_newsletter_group_subscriber.subscriber_id=tbl_newsletters.subscriber_id","left");
		  	$this->db->where("tbl_newsletter_group_subscriber.newsletter_groups_id",$newsletter_groups_id);
		  	$this->db->group_by("tbl_newsletter_group_subscriber.subscriber_id");
		 }
		 
		$this->db->where($where_condtion);
		
		$this->db->order_by("tbl_newsletters.subscriber_id","desc");		
	...
	public function change_status()
	{
			...
			if(is_array($arr_ids)){			
				...
				{				
					$query = $this->db->query("UPDATE tbl_newsletters SET user_status='1' WHERE subscriber_id IN ($str_ids)");
				...
				{				
					$query = $this->db->query("UPDATE tbl_newsletters SET user_status='0' WHERE subscriber_id IN ($str_ids)");
					$this->session->set_userdata(array('msg_type'=>'success'));
				...
				{				
					$query = $this->db->query("DELETE from tbl_newsletters WHERE subscriber_id in ($str_ids)");			
					$query = $this->db->query("DELETE from tbl_newsletter_group_subscriber WHERE subscriber_id IN ($str_ids)");	
					$this->session->set_userdata(array('msg_type'=>'success'));
				...
				if($this->input->post('add_group')==1 && $this->input->post('group_id')!="")
				{
					...
					foreach ($arr_ids as $id)
					{
						...
						if($this->check_subscriber($cond) ==0 )
						{
						      ...
						      $this->db->insert('tbl_newsletter_group_subscriber',
			...
			/* Unbind Subscriber in Groups */
				...
				if($this->input->post('add_group')==2 && $this->input->post('ugroup_id')!="")
				{
					foreach ($arr_ids as $id)
					{					
						$this->db->delete('tbl_newsletter_group_subscriber',
	...
	{	
		...
		{	 
			...
			{			
				$query = $this->db->query("SELECT * FROM tbl_newsletters WHERE subscriber_id='$value'");				
	...
	{
		$qry = $this->db->query("select subscriber_id from tbl_newsletters where 1 $cond");
		$num=$qry->num_rows();
		$qry->free_result();
		return $num;
	}
	...
	{
		$qry = $this->db->query("select subscriber_id from tbl_newsletter_group_subscriber where 1 $cond");
		$num=$qry->num_rows();
		$qry->free_result();
		return $num;
	}
	...
	public function get_group_email($subscriber_id="")
		{
			$this->db->select('tbl_newsletter_groups.group_name');
			$this->db->from('tbl_newsletter_group_subscriber');
			$this->db->where('subscriber_id',$subscriber_id);	
			 $this->db->join('tbl_newsletter_groups',
							 'tbl_newsletter_groups.newsletter_groups_id=tbl_newsletter_group_subscriber.newsletter_groups_id','INNER');
		...
		{		
			...
			if($subscriber_id!='' && is_numeric($subscriber_id)){			
				...
				$result = $this->find('tbl_newsletters',$fetch_config);
				return $result;		
			 }		
		}
