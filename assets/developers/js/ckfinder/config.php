...
if(preg_match('/(localhost|weblink4you.com)/',$host_name))
{
	preg_match("~^".$host_name."/([^/]+)(.*?)$~",$host_name.$_SERVER['PHP_SELF'],$matches);
	$baseUrl='http://'.$host_name."/".$matches[1];	
	
}elseif(preg_match('/(192.168.1.249)/',$host_name))
{
	preg_match("~^".$host_name."/kripanand/([^/]+)(.*?)$~",$host_name.$_SERVER['PHP_SELF'],$matches);
	$baseUrl='http://'.$host_name."/kripanand/".$matches[1];	
