...
class LightOpenID
{
    ...
    protected function request($url, $method='GET', $params=array())
    {
        if(function_exists('curl_init') && !ini_get('safe_mode') && !ini_get('open_basedir')) {
	        ...
	        //$resp=$this->request_curl($url, $method, $params);
	        //print_r($resp);exit;
    ...
    function validate()
    {
        ...
        //echo urldecode($this->data['openid_return_to']) .'======='.$this->returnUrl;
       //exit;

        if (urldecode($this->data['openid_return_to']) != $this->returnUrl) {
